...
export function Gateway({ markets }: { markets: { market: Market; currency: "EGP" | "MAD"; locales: string[] }[] }) {
  ...
  useEffect(() => { const saved = localStorage.getItem("mata3-context"); if (saved && /^\/(eg|ma)\/(ar|en|fr)$/.test(saved) && markets.some(item => saved.startsWith("/" + item.market + "/") && item.locales.includes(saved.split("/")[2]))) router.replace(saved); }, [router]);
